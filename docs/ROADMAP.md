...
- P3.4 AntigravityProvider — deps: P3.1, P3.3 — AC: `providers/antigravity.py` registered in `BUILTIN`, driving `agy` headlessly per ADR 0013 as revised by the spikes. Observable: `uv run pytest -q tests/unit/test_antigravity_provider.py` passes with the subprocess injected (every error branch, including the print timeout that reports SUCCESS, mapped to a **retryable** error before the missing-output case); and `uv run pytest -m integration tests/contract/` drives real generations, giving `5 passed, 3 skipped` with `agy` on PATH and skipping cleanly without it.
- P3.5 Provider commands — deps: P3.1, P3.2, P3.4 — AC: `thumbforge provider list|check|models|set-key`; `set-key` stores in keyring only and is the single writer of a secret. Observable: `provider check antigravity` prints binary, version, inherited plugins and an `auth` check resolved from `agy models`; `provider check nope` exits 3; `provider models fake` exits 0 reporting no model selection while an unhealthy provider exits 4 rather than printing an empty list; `uv run pytest -q tests/unit/test_cli_provider.py` covers all four commands with no network and no real keyring.
