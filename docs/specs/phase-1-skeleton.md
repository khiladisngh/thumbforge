...
| `thumbforge db vacuum`  | `VACUUM` + `PRAGMA wal_checkpoint(TRUNCATE)`                                                             | 0, 1 |
...
- `put` sniffs `mime`, `width`, `height` with Pillow when the bytes are an image; `bytes` is the file size; `kind ∈ {raw, final, reference, preview}`; `compliant`/`compliance_report_json` stay `NULL` until Phase 5 fills them.
- `ext` is derived from the sniffed MIME (`jpg`, `png`), never from the source filename.
