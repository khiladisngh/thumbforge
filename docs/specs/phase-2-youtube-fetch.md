...
- **S11** yt-dlp flat playlist fields — **resolved**, see `docs/spikes/ytdlp.md`. `id`, `title`, `duration`, `channel_id`, `channel`, `url`, `thumbnails` and `view_count` are always present per entry under `extract_flat="in_playlist"`. `playlist_index` is **absent**, so `PlaylistItemMeta.position` is derived from enumeration order. `description`, `timestamp` and `availability` are always `None`, so playlist-sourced `VideoMeta` rows keep their empty `description` and `published_at=None`; a per-video full extract fills them, and `fetch` must not fan out one per item.
