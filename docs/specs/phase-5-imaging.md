...
- **P5.3** `imaging/compliance.py`, `core/services/compliance.py`.
...
def check(path: Path, *, max_bytes: int = 2_097_152) -> ComplianceReport

# core/services/compliance.py
def finalize(raw: Path, layout: LayoutSpec, out: OutputSettings, ctx: OverlayContext, store: AssetStore) -> tuple[Asset, ComplianceReport]
    # fit → overlay → encode (jpeg quality / png) → re-encode at lower quality while bytes > max_bytes (floor quality 60) → check → store.put(kind="final")
```
...
5. `finalize` stores the final asset and the report; `asset.compliant` and `compliance_report_json` are set on the row. A non-compliant result raises `ComplianceError` (exit `5`) after the asset is stored, so the user can inspect it.
...
- `finalize` on a raw image that encodes to 2.4 MB at quality 90 lowers quality until ≤ 2,097,152 bytes and the stored asset's `compliant` is `1`.
...
- Unit: `fit_to` size matrix; `resolve_font` fallback order with `tmp_path` config dir; `check` matrix above using synthetic images from Pillow; `finalize` quality loop with a noise image.
