...
3. Per item: render prompt with `part_number`/`part_label` → `GenerationRequest` with `reference_images=(ref_path,)` → provider (Phase 3 retries) → `finalize` (Phase 5) → asset rows → iteration `completed`. Every step is logged with `run_id`, `iteration_id`, `provider` bound in contextvars.
